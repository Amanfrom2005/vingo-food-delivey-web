...
export const signUp = async (req, res) => {
    try {
        ...
        if(mobile.length < 10){ return res.status(400).json({ message: "Mobile number must be at least 10 digits long" })};

        const hasdPassword = await bcrypt.hash(password, 10);
        user = await User.create({ fullName, email, password:hasdPassword, mobile, role });
